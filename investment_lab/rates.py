...
def compute_forward(df_options: pd.DataFrame, df_rates: pd.DataFrame) -> pd.DataFrame:
    ...
    # Compute one interpolated rate per (date, expiration) pair, then merge back.
    df_curve = (
        df[["date", "expiration", "day_to_expiration", *tenor_cols]]
        .drop_duplicates(subset=["date", "expiration"])
        .copy()
    )
    tenors = np.array([TENOR_TO_PERIOD[t] for t in tenor_cols], dtype=float)
    df_curve["risk_free_rate"] = df_curve.apply(
        lambda r: interpolate_rates(
            eval_tenor=float(r["day_to_expiration"]) / DAYS_PER_YEAR,
            tenors=tenors,
            rate_curve=r[tenor_cols].to_numpy(dtype=float),
        ),
        axis=1,
    )
    df = df.merge(
        df_curve[["date", "expiration", "risk_free_rate"]],
        on=["date", "expiration"],
        how="left",
    )
